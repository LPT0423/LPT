...
```python
def heapify(nums, n, i):    #第一個函數(for將原始Array排序成Maxheap的要求,就是Parent節點值一定比Children節點值大)
    ...
    if largest != i:        #判斷這次的Parent與Children作比較後,是否Children的值比Parent大,有的話做交換
        ...
        heapify(nums, n, largest)  #這邊是做遞迴它call自己Heapify為何要作遞迴,因為有作互換,要再以此重作一次Maxheap  
  

    def heapSort(nums):               #第二個函數(for將Maxheap的結果排序成由小到大)
        n = len(nums)  
